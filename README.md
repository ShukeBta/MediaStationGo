...
硬链接不能直接链接“目录”本身，只能链接目录里的文件。文件管理器执行目录硬链接时会递归创建目标目录结构，并为每个文件创建硬链接。硬链接还要求源文件和目标文件在容器内属于同一个文件系统/子卷；如果下载目录和媒体目录是两个独立 bind mount、不同硬盘、不同 btrfs 子卷或网盘挂载，系统会返回 `invalid cross-device link`，此时可在媒体整理中选择“本地 STRM”，或按需使用“复制”“软链接”。

**多块硬盘如何统一整理并保种？**

在自动整理、手动整理或媒体详情的整理窗口中选择“本地 STRM（跨盘保留源文件）”。目标媒体库会按命名规则生成 `.strm`，指向源盘媒体的绝对路径；源媒体保留，NFO 复制到目标，支持整理后扫描和播放。API 对应 `transfer_mode: "strm"`。播放时源盘路径必须可访问；Docker 中需要挂载源盘目录，其他直接读取 STRM 的播放器也需要能访问文件中记录的路径。
